...
: "${BT_MAC:="$(hciconfig hci0 | grep 'BD Address' | cut -d' ' -f 3 | sed 's/\://g')"}"
: "${WLANID:="$(cat /sys/class/net/wlan0/address | sed 's/\://g')"}"
...
main() {
  ...
  if [ -z "${MLB_SERIAL_NUMBER}" ]; then
    die_with_error_message "'mlb_serial_number' not set in RO VPD."
  fi

  if [ -z "${BIOS}" ]; then
    die_with_error_message "'bios' not set in RO VPD."
  fi
  ...
  print_post_content NotifyEvent "${event}" \
    ...
    factory.bios "${BIOS}" \
    factory.bt_mac "${BT_MAC}" \
    factory.wlanid "${WLANID}" \
