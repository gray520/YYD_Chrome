...
# debug logs during the warm (cold) reboot  operations
...
function common_time_tag(){
	...
	local msg_ret=""
	local tmp_node=/usr/local/tmp_node
...
function common_diff_of_send_commands(){
	...
	local tmp_msg="$6"
	local tmp_node_name=/usr/local/tmp_node_${station_command_name}
...
function func_record_steps_after_reboot(){
	local actions=$1
	local logs_path=$2
	local timestamp=$3

	if [ "$actions" = "${NAME_STAGE_POST}" ]; then
        echo -e "\n\n(*) cbmem -c:$(cbmem -c)" >> ${logs_path}
	fi
}
...
function func_main_flow(){
	...
	# ---------------------------------------------------------------------------
	# record other-log for special reboot actions
	# ---------------------------------------------------------------------------
	func_record_steps_after_reboot "${actions}" "${logs_path}" "${timestamp}"
